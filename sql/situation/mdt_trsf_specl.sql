...
FROM EXECUTION%ANNEE%.LGECRITURE@dblcca2 L
	...
	SELECT  rownum r__ , niv2.* from (
		...
		from (SELECT  M.ASSIGNATAIRE,
			   ...
			   NVL(BT_TYPE_STATUS_LIB,CASE WHEN NVL(MAND_REJET,0) = 1 THEN 'REJET'
					...
					ELSE 'EN INSTANCE DE PRISE EN CHARGE' END) STATUT,T.*,E.*
		FROM EXECUTION%ANNEE%.MANDAT@dblcca2 M, EXECUTION%ANNEE%.ECRITURE@dblcca2 E, TRANSFERT T
