<?php
/* ***** BEGIN LICENSE BLOCK *****
 * Classe PhpToPdf modifié par Sogelis
...
class phpToPDF extends PDF_HTML
{
	...
	*/
	...
	{
            // ESA 12-08-2011
            // #386 Ajouté pour faire un saut de page
...
//			}
		...
		# j'include le titre du tableau dans la hauteur du header pour déterminé le changement de page
		...
		for($i=0; $i<$this->tb_columns; $i++)
		{
			//je regarde s'il y a une bordure a mettre sous l'entête
	...
	*/
	...
	function Draw_Data($data, $header = true){
		...
		while($cut_text && ($this->GetY()+$h3) > $this->PageBreakTrigger){
			//OPT CUT_TEXT : si le texte ne tient pas sur la page, je coupe le texte en deux lignes de cellules au lieu de passer à la page suivante
...
*/
		...
		//si le texte a été coupé, je recalcule la hauteur
		if($text_coupe){
			$h = 0;
			for($i=0;$i<$this->tb_columns;$i++)
			{
	...
	function MultiCellTable($w, $h, $txt, $border=0, $align='J', $valign='T', $fill=0, $vh=0)
	{
		...
		*/
		...
		if($txt != strip_tags($txt)) {
			...
			$t_image=array();
			$h_image=0;
				// Pour préciser une hauteur de cellule différente, il faut utiliser la balise de type [H5]
	...
	function drawTableau(&$pdf, $titre_rubrique, $tableType, $headerType, $headerDatas, $datasType, $datas, $minHeight="")
	{
		...
		for ($i=0 ; $i<count($datas) ; $i+=$nbCol)
		{
			...
			for ($j=$i ; $j<$i+$nbCol ; $j++)
			{
				...
				// Pour préciser une couleur de fond différente, il faut utiliser la balise de type [(202,204,222)]
				...
				// Pour préciser une taille de texte différente, il faut utilser la balise de type [F10]
				$tmp = preg_match('/\[([a-zA-Z0-9]*)F([0-9]+)([a-zA-Z0-9]*)\](\s*)/', $data[$k]['TEXT'], $m);
				if(isset($m[2])){
					$data[$k]['T_SIZE'] = $m[2];
					$data[$k]['TEXT'] = str_replace("F".$m[2]."", "", $data[$k]['TEXT']);
				}
				// Pour préciser une hauteur de cellule différente, il faut utilser la balise de type [H5]
