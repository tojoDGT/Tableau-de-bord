...
class PDFTable extends PDF_HTML{
...
function Footer(){
	...
	require_once(PATH_ROOT_DIR ."/pdf/phpqrcode/qrlib.php");
	...
	if($this->zUrl !=''){
		/*$oCode = new StdClass();
		$oCode->entite_denomination = $oSession[0]['entite_denomination'];
		$oCode->entite_adressePhysique = $oSession[0]['entite_adressePhysique'];
		$oCode->entite_postale = $oSession[0]['entite_postale'];
		$oCode->entite_telephone = $oSession[0]['entite_telephone'];
		$oCode->entite_mail = $oSession[0]['entite_mail'];
		$oCode->entite_sig = $oSession[0]['entite_sig'];
		$oCode->entite_siteWeb = $oSession[0]['entite_siteWeb'];
		$oCode->entite_slogan = $oSession[0]['entite_slogan'];*/

		$this->SetY(-20);

		$zTempDir = PATH_ROOT_DIR.'/assets/upload/resize/';
		$zFilePath = $zTempDir.'/'.uniqid();

		//QRcode::png('some text', $zFilePath);

		QRcode::png($this->zUrl,$zFilePath);

		$this->Image($zFilePath, 270, 195, 15, 15, "png");
	}
}
...
private function _tableWriteRow(&$table,$i,$x0,&$iMax){
	$maxh = $this->_getRowHeight($table, $i);

	$iMax += $maxh;
	//echo $table['lasty'] . " > " . $maxh . " --" . "-- this bottom :" . $this->bottom . $i . "<br>\n";
...
private function _tableWrite(&$table){
	...
	if (isset($table['nobreak'])&&$table['nobreak'] == 1){
		$this->AddPage($this->CurOrientation);
		$table['lasty'] = $this->y;
	}else
		$table['lasty'] = $y0;
	...
	for ($i=0;$i<$table['nr'];$i++) {
		$iMax = 0;
		$this->_tableWriteRow($table, $i, $x0,$iMax);
...
function htmltable(&$html,$multipage=1){
	...
	$this->SetAutoPageBreak(1,$this->bMargin);
...
private function getAlign($v){
	...
	$PDF_ALIGN1  = array('left'=>'L','center'=>'C','right'=>'R','justify'=>'J');
	$v = strtolower($v);
	return isset($PDF_ALIGN1[$v])?$PDF_ALIGN1[$v]:'L';
}
private function getVAlign($v){
	
	//global $PDF_VALIGN;

	$PDF_VALIGN1 = array('top'=>'T','middle'=>'M','bottom'=>'B');
	$v = strtolower($v);
	return isset($PDF_VALIGN1[$v])?$PDF_VALIGN1[$v]:'T';
}
}//PDFTable
?>